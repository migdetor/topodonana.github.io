---
layout: default
title: "Cartography - Cartografía"
permalink: /carto/
---
<!--
<img src="{{ "/assets/img/perezi.jpg" | relative_url }}" alt="Topo Doñana" style="max-width: 100%; height: auto; margin: 0 0 1rem 0;">
-->
# Cartographic information   Información cartográfica

[EN] This page brings together the cartographic layers and resources associated with the updated map of Doñana National Park. For each layer you will find a **Fact sheet** (documentation) and, when applicable, a **Download** link (repository / Zenodo).

[ES] Esta página reúne las capas cartográficas y recursos asociados al mapa actualizado del Parque Nacional de Doñana. Para cada capa encontrarás una **Ficha** (documentación) y, cuando aplique, un enlace de **Descarga** (repositorio / Zenodo).

---

## Quick access - Acceso rápido

**[EN]**
- **Full download** (stable package): [Zenodo](https://zenodo.org/records/XXXXX) · [Releases](https://github.com/USUARIO/REPO/releases)
- **Versions & changes** (changelog): [GitHub Releases](https://github.com/USUARIO/REPO/releases)

**[ES]**
- **Descarga completa** (paquete estable): [Zenodo](https://zenodo.org/records/XXXXX) · [Releases](https://github.com/USUARIO/REPO/releases)
- **Versiones y cambios** (changelog): [GitHub Releases](https://github.com/USUARIO/REPO/releases)
  

- **How to cite - Cómo citar**: [DOI](https://doi.org/10.XXXX/XXXXX) · [License]({{ "/licencias/" | relative_url }})  
 
---

## Scope & recommended use - Alcance y uso recomendado

**[EN]**
- **Spatial coverage**: Doñana National Park and immediate surroundings (specify boundaries if applicable).  
- **Reference system (CRS)**: EPSG:XXXX (e.g., ETRS89 / UTM Zone 30N).
- **Recommended use**: synthesis cartography and ecological/territorial analysis.  
  *Not suitable for cadastral delimitation or legal decisions.*
- [EN] **Effective resolution / scale**: integrated product ~1:XX,XXX (approx.). Each layer may have a different effective scale (see fact sheets).
- **Temporal coverage**: Version V1 (2026). Layer-specific temporal components are documented per layer.  

**[ES]**
- **Cobertura espacial**: Parque Nacional de Doñana y entorno inmediato (especificar límites si aplica).
- **Sistema de referencia (CRS)**: EPSG:XXXX (p. ej., ETRS89 / UTM zona 30N).
- **Uso recomendado**: cartografía de síntesis y análisis ecológico/territorial.  
  *No apto para deslindes ni decisiones jurídicas.*
- **Resolución / escala efectiva**: producto integrado ~1:XX.XXX (aprox.). Cada capa puede tener su propia escala efectiva (ver fichas).
- **Cobertura temporal**: versión V1 (2026). Componentes temporales específicos se documentan por capa.

---

## Main layers - Capas principales

**[EN]**

| Layer | Fact sheet | Download |
|---|---|---|
| **Doñana toponyms** | [Fact sheet]({{ "/toponimos/" | relative_url }}) | [Download](https://zenodo.org/records/XXXXX) |
| **Pond network cartography** | [Fact sheet]({{ "/lagunas/" | relative_url }}) | [Download](https://zenodo.org/records/XXXXX) |
| **Cork oak tree cartography** | [Fact sheet]({{ "/alcornoques/" | relative_url }}) | [Download](https://zenodo.org/records/XXXXX) |
| **Flooding duration (hydroperiods)** | [Fact sheet]({{ "/hidroperiodos/" | relative_url }}) | [Download](https://zenodo.org/records/XXXXX) |

**[ES]**

| Capa | Ficha | Descarga |
|---|---|---|
| **Toponimia de Doñana** | [Ficha]({{ "/toponimos/" | relative_url }}) | [Descarga](https://zenodo.org/records/XXXXX) |
| **Cartografía de la red de lagunas** | [Ficha]({{ "/lagunas/" | relative_url }}) | [Descarga](https://zenodo.org/records/XXXXX) |
| **Alcornoques del PN Doñana** | [Ficha]({{ "/alcornoques/" | relative_url }}) | [Descarga](https://zenodo.org/records/XXXXX) |
| **Duración de la inundación (hidroperiodos)** | [Ficha]({{ "/hidroperiodos/" | relative_url }}) | [Descarga](https://zenodo.org/records/XXXXX) |

---

## Secondary layers - Capas secundarias

**[EN]** 
> Note: these layers may be incomplete or interpretative. Please read the fact sheet before use.  

| Layer | Fact sheet | Download |
|---|---|---|
| **Roads, tracks & firebreaks network** | [Fact sheet]({{ "/caminos/" | relative_url }}) | — |
| **Fences** | [Fact sheet]({{ "/vallados/" | relative_url }}) | — |
| **Buildings & constructions** | [Fact sheet]({{ "/edificaciones/" | relative_url }}) | — |

**[ES]**
> Nota: estas capas pueden ser incompletas o interpretativas. Consulta la ficha antes de su uso.

| Capa | Ficha | Descarga |
|---|---|---|
| **Red de caminos, carriles y cortafuegos** | [Ficha]({{ "/caminos/" | relative_url }}) | — |
| **Vallados y cercados** | [Ficha]({{ "/vallados/" | relative_url }}) | — |
| **Edificaciones y construcciones** | [Ficha]({{ "/edificaciones/" | relative_url }}) | — |

---

## Reference data - Datos de referencia

**[EN]** 
> These sources are external. If you publish derivatives (slopes, hillshade, mosaics), clearly state the origin and processing.  

| Layer | Fact sheet | Download |
|---|---|---|
| **Digital Terrain Model (DTM)** | [Fact sheet]({{ "/mdt/" | relative_url }}) | [CNIG/IGN](https://www.cnig.es/) |
| **Slope map** | [Fact sheet]({{ "/pendientes/" | relative_url }}) | [CNIG/IGN](https://www.cnig.es/) |
| **LiDAR** | [Fact sheet]({{ "/lidar/" | relative_url }}) | [CNIG/IGN](https://www.cnig.es/) |

**[ES]** 
> Estas fuentes son externas. Si publicas derivados (pendientes, sombreado, mosaicos), indica claramente el origen y el proceso.

| Recurso | Ficha | Fuente |
|---|---|---|
| **Modelo Digital del Terreno (MDT)** | [Ficha]({{ "/mdt/" | relative_url }}) | [CNIG/IGN](https://www.cnig.es/) |
| **Mapa de pendientes** | [Ficha]({{ "/pendientes/" | relative_url }}) | [CNIG/IGN](https://www.cnig.es/) |
| **LiDAR** | [Ficha]({{ "/lidar/" | relative_url }}) | [CNIG/IGN](https://www.cnig.es/) |

---

## License & contact - Licencia y contacto

**[EN]**
- **License**: see [License]({{ "/licencias/" | relative_url }}).
- **Suggestions / corrections**: email [here](mailto:m.defelipe.t@gmail.com).
  
**[ES]** 
- **Licencia**: consulta [Licencias]({{ "/licencias/" | relative_url }}).
- **Sugerencias / correcciones**: escríbeme [aquí](mailto:m.defelipe.t@gmail.com).
